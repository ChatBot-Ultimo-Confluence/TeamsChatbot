...
{
    /// <summary>
    /// Service responsible for generating text embeddings by calling an external embedding API (ROP style).
    /// </summary>
    public class EmbeddingService
    {
        ...
        /// <summary>
        /// Generates an embedding vector for the given text (wrapped in Result).
        /// Calls the Ollama embedding API running locally.
        /// </summary>
        public async Task<Result<float[]>> GenerateEmbeddingAsync(string text)
        {
            if (string.IsNullOrWhiteSpace(text))
                return Result<float[]>.Fail("Input text cannot be null or empty.");
            ...
            try
            {
                ...
                if (!response.IsSuccessStatusCode)
                    return Result<float[]>.Fail($"API call failed: {response.StatusCode}");
                ...
                if (!json.TryGetProperty("embedding", out var embeddingElement))
                    return Result<float[]>.Fail("Response JSON does not contain 'embedding' property.");
                ...
                return Result<float[]>.Ok(embeddingArray);
            }
            catch (HttpRequestException httpEx)
            {
                return Result<float[]>.Fail($"HTTP request failed: {httpEx.Message}");
            }
            catch (JsonException jsonEx)
            {
                return Result<float[]>.Fail($"JSON parsing failed: {jsonEx.Message}");
            }
            catch (Exception ex)
            {
                return Result<float[]>.Fail($"Unexpected error generating embedding: {ex.Message}");
            }
        }
    }
}
