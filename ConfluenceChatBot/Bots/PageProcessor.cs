...
{
    /// <summary>
    /// Processes Confluence pages with ROP:
    ...
    public class PageProcessor
    {
        ...
        /// </summary>
        public async Task<Result<bool>> ProcessPageAndInsertAsync(string pageId)
        {
            return await _confluenceService.GetPageContentAsync(pageId) // returns Result<ConfluencePage>
                .BindAsync(async page =>
                {
                    if (string.IsNullOrWhiteSpace(page.Content))
                        return Result<bool>.Fail("No content fetched from Confluence.");

                    var insertResult = await _pgVectorService.InsertEmbeddingOptimizedAsync(
                        page.Id, page.Title, page.Content, page.Version);

                    if (!insertResult.IsSuccess)
                        return Result<bool>.Fail(insertResult.Error);

                    Console.WriteLine($"[Inserted] Page {page.Title} (v{page.Version})");
        ...
        /// </summary>
        public async Task<Result<string>> SearchSemanticDataAsync(string query)
        {
            if (string.IsNullOrWhiteSpace(query))
                return Result<string>.Fail("Query is required.");

            return await _pgVectorService.SearchSimilarAsync(query, 10) // Result<IEnumerable<(section, content)>>
                .BindAsync(async results =>
                {
                    if (!results.Any())
                        return Result<string>.Fail("No matching information found in documents.");
                    ...
                    try
                    {
                        ...
                        return string.IsNullOrWhiteSpace(answer)
                            ? Result<string>.Fail("No answer generated.")
                            : Result<string>.Ok(answer);
                    }
                    catch (Exception ex)
                    {
                        return Result<string>.Fail($"Semantic search failed: {ex.Message}");
                    }
                });
        }
    }
