...
namespace ConfluenceChatBot.Controllers
{
    ...
    public class PageController : ControllerBase
    {
        ...
        {
            var result = await _pageProcessor.ProcessPageAndInsertAsync(pageId);
        ...
        {
            var result = await _pageProcessor.SearchSemanticDataAsync(query);

            if (!result.IsSuccess)
            {
                return result.Error switch
                {
                    "Query is required." => BadRequest(result.Error),
                    "No matching information found in documents." => NotFound(result.Error),
